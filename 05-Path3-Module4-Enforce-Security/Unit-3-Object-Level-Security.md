---
title: "Unit 3 — Apply object-level security"
module: DP-600
unit: 3 of 7
xp: 100
duration_minutes: 8
source: https://learn.microsoft.com/en-us/training/modules/enforce-semantic-model-security/3-object-level-security
tags:
  - dp-600
  - ols
...
# Unit 3 — Apply object-level security
...
> [!quote] Key property
> "OLS secures an object by hiding it AND hiding its metadata. A report can't accidentally reference a column the user doesn't know exists."

## 🎯 When to use OLS

OLS is designed for scenarios where certain data elements contain sensitive information that some users shouldn't access. Common use cases:
...
**Example** — HR semantic model where the `Employee` table contains a `Salary` column. General managers can view employee names and contact details, but only HR payroll staff should see salary values. OLS lets you hide the `Salary` column from all roles except the payroll role.

```mermaid
flowchart LR
    E["Employee table"] --> N["Name<br/>(Read for all)"]
    E --> C["Contact details<br/>(Read for all)"]
    E --> S["Salary<br/>(OLS: None for non-payroll)"]
```

> [!warning] OLS hides metadata too
> Unlike a visual-level filter, OLS removes the column from the model entirely for users in that role. They can't see it in the field list, can't reference it in measures (any measure referencing it fails), and AI surfaces like Copilot can't surface it.

## 🛠️ Configure OLS using Tabular Editor
...
> Assign members to roles in the Power BI service the same way you do for RLS roles. Navigate to the semantic model, select **More options (...)**, and then select **Security** to manage role membership.
...
**Example** — a `SalaryBand` lookup table used only for payroll calculations can be hidden from all roles except the payroll team.

> [!tip] Use table-level OLS for "lookup data only"
> Lookup/bridge tables that exist solely to support a specific role's calculations are ideal candidates for table-level OLS — hide the whole table for everyone else.
...
| Pattern | When to use |
|---------|-------------|
| Hide entire table | Lookup tables, staging, dev-only artifacts |
| Hide specific columns | Sensitive fields (PII, salary) inside an otherwise-visible table |
...
> [!important] If OLS would cause a poor user experience for some report consumers, consider creating separate reports or semantic models for different audience groups. A report that's designed for the audience avoids confusing error messages.
...
```mermaid
flowchart TD
    U["User"] --> RLS["RLS role<br/>filters rows"]
    U --> OLS["OLS role<br/>hides objects"]
    RLS --> Q["Effective query:<br/>visible rows × visible cols"]
    OLS --> Q
```

> [!warning] OLS extends to AI
> OLS prevents **Copilot** and **Fabric data agents** from surfacing restricted columns in natural language answers. When a column is secured with OLS, AI-powered features can't access or display that data, even through conversational queries.

## 🔑 Key terms (flashcards)

- **Object-level security (OLS)** — permission setting (`Read` / `None`) on a table or column, scoped per role.
- **Tabular Editor** — external Analysis Services modeling tool required to define OLS rules.
- **None permission** — hides the table or column AND its metadata from the role.
- **Read permission** — default; object and metadata are visible.
- **Relationship chain rule** — you cannot OLS-hide an entire table that sits in the middle of a relationship chain between other tables.

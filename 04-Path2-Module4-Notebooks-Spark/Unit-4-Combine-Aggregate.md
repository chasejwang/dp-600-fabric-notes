---
title: "Unit 4 — Combine and aggregate data"
module: DP-600
unit: 4 of 8
xp: 100
duration_minutes: 10
source: https://learn.microsoft.com/en-us/training/modules/fabric-transform-data-notebooks/4-combine-aggregate
tags:
  - dp-600
  - microsoft-fabric
  - notebook
...
# Unit 4 — Combine and aggregate data

> [!quote] Source
> Microsoft Learn · Path 2 · Module 4 · Unit 4 · "Combine and aggregate data"
> <https://learn.microsoft.com/en-us/training/modules/fabric-transform-data-notebooks/4-combine-aggregate>

## 🎯 Purpose

Build on the **10 cleaned sales rows** from [[Unit-3-Shape-Clean-Data]] and learn to **combine data from multiple tables**, **summarize it into aggregated metrics**, **compute values across related rows without collapsing the detail**, and **pivot rows into columns** for cross-tabulation. These patterns form the core of most analytics transformations.
...
> [!info] What an inner join does
> Before the join, the sales table had `customer_id` values like `"C001"` but no customer names. After the inner join, each row now includes the matching `customer_name` and `region`. If a sales row had a `customer_id` that didn't exist in the customers table, an inner join would **exclude it**.
...
> [!tip] Most fact-to-dimension joins
> For most fact-to-dimension joins, **inner or left joins** are appropriate. Choose based on whether unmatched fact rows should be dropped or kept (with null dimension columns).
...
The 10 detail rows collapse into **one row per region**:
...
The `PARTITION BY` clause defines the group of rows for the calculation. The `ORDER BY` clause determines the sequence within each partition.
...
Common table expressions let you **break complex queries into named, readable steps**. Each CTE defines a temporary result set you can reference in subsequent steps. In PySpark, you achieve the same result by **chaining DataFrame operations**, where each variable holds an intermediate result.
...
The CTE approach first summarizes sales by month, then ranks the months. **Only the top 5 months by total sales survive the final filter.**
...
Pivoting **reshapes data from rows to columns**, which is useful for creating cross-tabulation summaries. For example, transform quarterly sales rows into columns for side-by-side comparison. **Specifying the pivot values** (like `Q1` through `Q4`) improves performance because Spark doesn't need to scan the data first to discover the distinct values.
...
## 🧠 Visual — combine-and-aggregate toolkit

```mermaid
flowchart TB
    subgraph Comb["Combine"]
      J[Join<br/>inner · left · right · full · cross]
    end
    subgraph Aggr["Aggregate"]
      G[GROUP BY<br/>+ HAVING]
      W[Window functions<br/>row_number · rank · lag · running totals]
      P[Pivot<br/>rows → columns]
      CTE[CTEs<br/>named, chained steps]
    end
    Inputs[(Multiple source tables)] --> J
    J --> Cleaned[(Cleaned, enriched rows)]
    Cleaned --> G
    Cleaned --> W
    G --> Summary[(Aggregated metrics)]
    W --> Detail[(Detail rows + window cols)]
    P --> Crosstab[(Cross-tabulation)]
    G --> CTE
    W --> CTE
    CTE --> Final[(Top-N, ranked result)]
```

## 🔑 Key takeaways

- **Joins** combine rows from related tables; Spark supports inner, left, right, full outer, and cross.
- **`GROUP BY` + aggregations** (`COUNT`, `SUM`, `AVG`) collapse rows into summary metrics; filter aggregated results with `HAVING` (SQL) or `.filter()` after the agg (PySpark).
- **Window functions** calculate values across related rows **without collapsing detail** — use for rankings, running totals, and row-to-row comparisons.
- **CTEs** (`WITH … AS`) make complex SQL readable; in PySpark, **chain DataFrame operations** into intermediate variables for the same effect.
- **Pivots** reshape rows into columns for cross-tabulation; **specify the pivot values explicitly** for better performance.
- Always **build up incrementally** — test each step in its own cell before chaining.
